...
parameters {
  ...
  real<lower=-0.025, upper = 0.025> opposition;
  real<lower=-0.025, upper = 0.025> government;
...
model {
  ...
  y_start ~ normal(-1, 3);
  government ~ normal(0, 0.0025);
  opposition ~ normal(0, 0.0025);
  alpha ~ normal(0.75, 0.5);
  theta ~ normal(0.9, 0.1);
  phi ~ normal(0.5, 0.5);
  ...
  Eps_corr ~ lkj_corr_cholesky(2);
  EpsPoll_corr ~ lkj_corr_cholesky(2);
