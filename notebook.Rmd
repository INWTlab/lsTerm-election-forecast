...
$$poll_{pt, t, pll}^* \sim N(y_{pt, t} + bias_{pll} + \epsilon_{\text{pollError}_{pt, t}}; \text{ } \sigma_{pll} + 1E-4)$$
...
$$\epsilon_{\text{pollError}_{pt, t}} = 0.98 \cdot \epsilon_{\text{pollError}_{pt, t-1}} + $$

$$y_{pt, t} = y_{pt, t-1} + \epsilon_{pt, t} + \nu_{pt, t} - \eta_{pt, t}$$

The shocks in vote's share follow a t-distribution with five degrees of freedom. The expectation $\mu_{pt, t}$ depends on the current state, i.e. state of government or opposition of the party, whereas the standard deviation $\sigma_{\text{shift}_{pt}}$ is different for each party $pt$:
...
The negative short term effect $\eta_{pt, t}$ follows an AR1-type process:
