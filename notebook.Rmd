...
The true vote share $y_{pt, t}$ws a random walk (with drift), but two additional terms 𝜐_(𝑝𝑡,𝑡) and 𝜂_(𝑝𝑡,𝑡), the short and long-term memory effects:

$$y_{pt, t} = y_{pt, t-1} + \epsilon_{pt, t} + \nu_{pt, t} - \eta_{pt, t}$$

The shocks in vote share follow a t-distribut $\epsilon_{pt, t}$ion with five degrees of freedom. The expectation $\mu_{pt, t}$ depends on the current state, i.e. state of being in government or opposition for the party, whereas the standard deviation $\sigma_{\text{shift}_{pt}}$ is different for each party $pt$:
