...
## Results
### Visualization
Six months before election, the social democratic party ("SPD") experienced a large increase (from slightly over 20\% to more than 30\%  in the polls) in voter's sharedue to a hype of the shortly announced chancellor candidate Martin Schulz. From the picture below, we can clearly see a mean-reversing trend until election date for the "SPD" (in red):
...
ggplot(data = plotData, aes(x = time, y = estimate, group = party,
  ...
  annotate(geom = "text", x=as.POSIXct(predDate), y=0,
...
Now, we evaluate the performance on a monthly basis from 12 months before election until the election week:

## Troubleshooting
The iterations exceeds maximum tree size, for tree_depth <18 (but results are ok for tree_depth larger than 15). Fortunately, there are no divergent samples for adapt_stepsize value of 0.9. The models have been running with rather low sample size and 

## Outlook
### Model speed up 
The model is quite slow and about 1 week to compute
  - Use map_rect on party level to speed up computation
  - Faster computation with future *STAN* versions, model could benefit from faster indexin in *STAN* 2.20
  - Potentially re-parameterize model
