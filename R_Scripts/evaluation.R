...
cbind(electionYear, sapply(electionYear, function(x) {
  if (x == 1998) {
  ...
  mean(diff(apply((
    dataDE$pollData %>% 
      filter(
        .data$Datum >= as.Date(paste0(x, "-01-01")) & .data$Datum <= maxDate)
  )[, 3:7], 2, range)))
})) -> res
...
volatility_polls_plot <- res %>%
  ...
  labs(title = "Schwankungen der Umfragewerte bei den letzten Bundestagswahlen",
       subtitle = "Durchschnittliche Spannweite der Umfragewerte im jeweiligen Wahljahr in Prozentpunkten") +
  ylab("Prozentpunkte") +
...
polls <- loadDataDE(predDate)$pollData
polls <- polls %>% arrange(desc(Datum))
...
for (x in 1:length(evaluation_dates)) {
  forecast_eval <- forecast %>%
    filter(type == "election_day",
           date_forecast == max(forecast$date_forecast[forecast$date_forecast <= (evaluation_dates[x])]))
  ...
  last_polls <-
    polls %>% filter(Datum == last_polls_date) %>% select("CDU/CSU", "SPD", "GRÜNE", "FDP", "LINKE", "AfD")
  last_five_polls <-
    polls %>% filter(Datum <= evaluation_dates[x] &
                       Datum > (as.Date(evaluation_dates[x]) - 14)) %>%
    group_by(Institut) %>% slice(1) %>% ungroup %>% select("CDU/CSU", "SPD", "GRÜNE", "FDP", "LINKE", "AfD")
  last_five_polls <-
    last_five_polls[1:min(5, NROW(last_five_polls)),] %>% colMeans
  
  # Evaluation:
  
  abs_error[x, 1] <- mean(abs(result - forecast_eval))
  abs_error[x, 2] <-
    rowMeans(abs(as.matrix(result)[rep(1, nrow(last_polls),), ] - as.matrix(last_polls))) %>% mean
  abs_error[x, 3] <- mean(abs(result - last_five_polls))
  
  sq_error[x, 1] <- mean(abs(result - forecast_eval) ^ 2) %>% sqrt
  sq_error[x, 2] <-
    (rowMeans(abs(
      as.matrix(result)[rep(1, nrow(last_polls),), ] - as.matrix(last_polls)
    ) ^ 2) %>% sqrt %>% mean)
  sq_error[x, 3] <-
    (mean(abs(result - last_five_polls) ^ 2) %>% sqrt)
  ...
  rel_error[x, 2] <-
    rowMeans(abs((as.matrix(log(
      ...
      last_polls
    ))) / as.matrix(log(result))[rep(1, nrow(last_polls),), ])) %>% mean
  rel_error[x, 3] <-
