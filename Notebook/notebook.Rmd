...
3. The polling uncertainty of a specific pollster for a specific party (sampling error)
...
knitr::kable(head(dataDE$pollData %>% arrange(desc(Datum)) %>% 
                    filter(Datum < as.Date("2005-09-18"))))
...
$$logit(poll_{pt, t, pll}) \sim N(\mu_{pt, t, pll}\text{ , } \sigma^2_{pt,pll} + 0.0002)$$
...
For an election, pollster specific bias and variance is non-existent. The upper expression gets simplified to the one below. The small standard deviation is used to avoid problems with the sampler and can also be interpreted as counting error.

$$logit(election_{pt, t}) \sim N(y_{pt, t}\text{ , } 0.0002)$$

The shifts in the common bias of the pollsters for all parties follow a normal distribution with mean 0 and a standard deviation $\sigma_{\text{pollbias}}$ that is different for each party $pt$ multiplied by a correlation matrix $\Sigma_{shift}$:
$$\epsilon_{\text{polls}_{pt, t}} \sim multi_normal(0, \sigma^2_{\text{pollbias}_{pt}} \Sigma_{shift})$$
...
$$\epsilon_{pt, t} \sim multi_t(\mu_{eps_{pt, t}}, \sigma_{\text{shift}_{pt}}\Sigma_{shift}, df = 3.5)$$
...
\medskip
```{r stanCode, warning=FALSE, echo = TRUE, message=FALSE, eval = FALSE, output.var = ""}
parameters {
...
```{r stanCode2, warning=FALSE, echo = TRUE, message=FALSE, eval = FALSE, output.var = " "}
data {
...
biasAllensbach <- 
  modelResults$samples$housebias[, grep("Allensbach",
...
## Troubleshooting
The model iterations exceed maximum tree size, for tree_depth < 13. As each additional tree_depth increases the computation time about two-fold, this leads to slow computation. There were, very rarely, divergent samples. Therefore, the user might increase adapt_stepsize above the value of 0.8 in edge cases (the results are virtually identical, though). To allow for daily updates models has been running with rather low sample size (600 with 400 burn-in) with 5 chains in practice. For more precise results, more chains or longer sampling periods are desirable.
\bigskip
...
To keep the Bayesian workflow, a dirichlet prior was chosen with all $\alpha_k$ set to 0.5 ('non-informative' Jeffrey's prior), which changes the results slightly. Another small change to the setting described above was to introduce senior and junior partners, e.g. in the upper example for coalition A+B the expert were given two choices, namely with A as senior partner and B as senior partner, who provide the chancellor in each case. As (at least in Germany) there is an unwritten rule that the party with the most votes provides the chancellor, both coalitions rule out each other and the ranking between them is irrelevant. With this assumption one can also estimate the probability for a certain person being chancellor. In general, as coalition majorities are negatively correlated (e.g. A+B and C+D cannot have a majority at the same time), usually a low double-digit sample of experts already leads to high certainty in probability estimates. Besides the most voted party providing the chancellor, another assumption that is made concerns minority governments, which are ruled out. These are completely unusual in Germany (both on federal and state level), but for other countries with similar election system not unheard of and would require non-trivial model adjustments.

The approach explained above only considers the variation between experts but ignores the uncertainty within a single expert. More specifically, we assume that an expert is 100% sure, that coalition A+B will be chosen instead of coalition A+C if he ranked A+B above A+C. In reality he might only expect that to be case with a probability of 70%, 80% or 90%. To incorporate this uncertainty, we additionally model the expert's judgement and sample repeatedly afterwards from it. How does this work in our example? Say in one of our parliament set share simulation, coalitions A+B, A+C and A+B+C (scenario 2. above) have a majority. As a prior -- from a single expert view-- for the chance that a specific coalition is selected we again use the dirichlet distribution with all $\alpha_k$ set to 0.3 (derived as optimal ML-value by analysis of previous expert survey and election results of the German election in 2017, alternatively one can also choose Jeffrey's prior with $\alpha_k = 0.5$ again). When we incorporate the ranking, i.e. the knowledge that P(A+B) > P(A+C) > P(A+B+C), our posterior is a restricted dirichlet distribution. To sample from it, we can just sample from a standard dir(0.3, 0.3, 0.3) and order by the returned samples. That gives us probabilities of ca. 76.8\% for coalition A+B, 19.4\% for coalition A+C and 3.7\% for coalition A+B+C. However, we impose a second restriction: If there is coalition with less parties possible (i.e. having majority), the probability of the coalition having a higher number of parties is set to 0. In our example we set P(A+B+C) to 0, as we already have two coalitions with a parliament majority that have less parties and which are all included within the three-party coalition. This would of course not apply when we have the choice between e.g. A+B and A+C+D. Coming back to our example, we then have probabilities of 86.7\% for A+B and 13.3\% for A+C.

The package includes a function with pre-defined coalitions to compute their probabilities. The big coalition (CDU/CSU + SPD) got by far the highest probability assigned (and eventually became government after election):

```{r coalitions eval, warning=FALSE, echo = TRUE, message=FALSE}
...
coalition_parties <- c("CDU/CSU - SPD", "SPD - Linke - Grüne", "SPD - FDP-Grüne",
                ...
                "Grüne - CDU/CSU - FDP", "CDU/CSU-SPD-FDP", "SPD - CDU/CSU - FDP",
                "CDU/CSU - Grüne - SPD", "Grüne - CDU/CSU - SPD",
                "SPD - CDU/CSU - Grüne")
...
With including the expert uncertainty extension, we get a bit more balanced, but overall very similar results:

```{r coalitions2 eval, warning=FALSE, echo = TRUE, message=FALSE}
fact_coalition_prob <- koalitionDE(dataDE$Koalitionen, modelResults, dataPrep, predDate,
                                   expertUncertainty = TRUE)
coalition_parties <- c("CDU/CSU - SPD", "SPD - Linke - Grüne", "SPD - FDP-Grüne",
                "CDU/CSU - Grüne", "SPD - Grüne", "SPD - CDU/CSU", 
                "CDU/CSU - FDP", "CDU/CSU - FDP - Grüne", "Grüne - SPD",
                "Grüne - SPD - Linke", "Grüne - SPD - FDP", "Grüne - CDU/CSU", 
...
It may also happen, that the voting share of the parties changes substantially after performing the expert survey and improbable coalitions now get somewhat conceivable. This could include a coalition such as CDU/CSU - SPD - FDP or CDU/CSU - Grüne - SPD for the German Bundestag elections. These can be incorporated by sampling their expert rank from an discrete uniform distribution. One can take advantage of the available expert data by restricting the range of ranks such that. For example, when considering the additional coalition CDU/CSU - Grüne - SPD, we can conclude that the rank must be larger than the rank of CDU/CSU - Grüne and CDU/CSU - SPD. This leads to higher uncertainties but avoids having to repeat the expert survey coupled with significant effort.
